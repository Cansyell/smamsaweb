...
@section('title', 'Edit Kuota Spesialisasi')
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="mb-6">
        <div class="flex items-center gap-3 mb-2">
            ...
            <div>
                <h2 class="text-2xl font-bold text-gray-800">Edit Kuota Spesialisasi</h2>
                <p class="text-sm text-gray-600 mt-1">Perbarui data kuota untuk {{ $specializationQuota->academicYear->year }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-md">
        <form action="{{ route('admin.specialization-quotas.update', $specializationQuota) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
                ...
                <select name="academic_year_id" 
                    ...
                    @foreach($academicYears as $year)
                        <option value="{{ $year->id }}" 
                            {{ old('academic_year_id', $specializationQuota->academic_year_id) == $year->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="number" 
                            name="tahfiz_quota" 
                            id="tahfiz_quota" 
                            value="{{ old('tahfiz_quota', $specializationQuota->tahfiz_quota) }}"
                            min="0" 
                            max="1000"
                            placeholder="0"
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="number" 
                            name="language_quota" 
                            id="language_quota" 
                            value="{{ old('language_quota', $specializationQuota->language_quota) }}"
                            min="0" 
                            max="1000"
                            placeholder="0"
            ...
            <div class="mb-6 p-4 bg-indigo-50 border border-indigo-200 rounded-lg">
                <div class="flex items-center justify-between">
                    ...
                    <span class="text-2xl font-bold text-indigo-600" id="total-quota">{{ $specializationQuota->total_quota }} siswa</span>
                </div>
                ...
                <div class="flex items-start">
                    <div class="flex items-center h-5">
                        <input type="checkbox" 
                            ...
                            id="is_active" 
                            value="1"
                            {{ old('is_active', $specializationQuota->is_active) ? 'checked' : '' }}
            ...
            <div class="mb-6 p-4 bg-gray-50 border border-gray-200 rounded-lg">
                ...
                <div class="grid grid-cols-3 gap-4 text-sm">
                    ...
                    <div>
                        ...
                        <p class="font-semibold text-indigo-600">{{ $specializationQuota->total_quota }} siswa</p>
                    </div>
                </div>
            </div>
