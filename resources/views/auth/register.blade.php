        ...
        <div class="bg-white rounded-lg shadow-xl p-8">
            ...
            <div class="text-center mb-8">
                <div class="inline-flex items-center justify-center mb-4">
                    <img src="{{ asset('images/logo-smamsa1.jpeg') }}" alt="Logo SMAMSA" class="w-16 h-16 object-contain">
                </div>
            ...
            @if (!$registrationOpen)
                {{-- Pendaftaran ditutup / belum ada tahun ajaran aktif --}}
                <div class="text-center py-6">
                    ...
                    <h3 class="text-lg font-semibold text-gray-800 mb-2">Pendaftaran Ditutup</h3>
                    <p class="text-gray-600 text-sm mb-6">
                        @if ($activeYear)
                            Periode pendaftaran tahun ajaran {{ $activeYear->full_year }} telah berakhir pada {{ $activeYear->end_date->translatedFormat('d F Y') }}.
                        @else
                            Saat ini belum ada periode pendaftaran yang aktif. Silakan hubungi admin untuk informasi lebih lanjut.
                        @endif
                    </p>
                    <a href="{{ route('login') }}" class="inline-block px-6 py-3 bg-purple-600 text-white rounded-lg font-semibold hover:bg-purple-700 transition">
                        Kembali ke Login
                    </a>
                </div>
            @else
                {{-- Pesan error dari store() kalau ada yang coba submit paksa saat periode udah tutup --}}
                @if ($errors->has('registration'))
                    <div class="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg">
                        <p class="text-sm text-red-600">{{ $errors->first('registration') }}</p>
                    </div>
                @endif
