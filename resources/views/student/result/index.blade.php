        ...
        <div class="p-6">
            ...
            <div class="space-y-4">
                @if($filterSpecialization === 'regular')
                    ...
                    <div class="border border-gray-200 rounded-lg p-4 hover:border-blue-300 transition {{ $student_item->id == $student->id ? 'bg-blue-50 border-blue-400 ring-2 ring-blue-200' : '' }}">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center space-x-4 flex-1">
                                <!-- Position Badge -->
                                <div class="flex-shrink-0">
                                    @if(($rankings->currentPage() - 1) * $rankings->perPage() + $index + 1 <= 3)
                                        <div class="flex items-center justify-center w-12 h-12 rounded-full {{ ($rankings->currentPage() - 1) * $rankings->perPage() + $index + 1 == 1 ? 'bg-yellow-400' : (($rankings->currentPage() - 1) * $rankings->perPage() + $index + 1 == 2 ? 'bg-gray-300' : 'bg-orange-400') }} text-white font-bold text-lg">
                                            {{ ($rankings->currentPage() - 1) * $rankings->perPage() + $index + 1 }}
                                        </div>
                                    @else
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center gap-2 mb-1">
                                        ...
                                        @if($student_item->id == $student->id)
                                            <span class="px-2 py-1 text-xs font-medium bg-blue-500 text-white rounded-full animate-pulse">
                                                ← Posisi Anda
                                            </span>
                                        @endif
                                    </div>
